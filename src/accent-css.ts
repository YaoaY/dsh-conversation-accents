...
import type { AccentPalette, SemanticColors } from "./palettes.js";

export const STYLE_PLUGIN = "dsh-conversation-accents" as const;
export const MARKDOWN_ACCENT_STYLE_KIND = "assistant-markdown-accents" as const;
export const TOOL_ACCENT_STYLE_KIND = "tool-accents" as const;
export const THINK_ACCENT_STYLE_KIND = "think-accents" as const;
export const THINK_MARKDOWN_STYLE_KIND = "think-markdown-layout" as const;
export const THINK_MARKDOWN_ATTRIBUTE = "data-dsh-think-markdown" as const;

const ACCENTED_TOOL_NAMES = ["pwsh", "read", "edit", "write", "grep", "glob", "web_search"] as const;
const GENERIC_TOOL_SELECTOR = `:is(${ACCENTED_TOOL_NAMES.map((tool) => `[data-tool="${tool}"]`).join(", ")})`;

export const TOOL_ACCENT_CSS = [
...
export const THINK_MARKDOWN_CSS = [
...
function paletteVariables(colors: SemanticColors): string {
  return [
...
function modeCss(colors: SemanticColors, prefix = ""): string {
  return `${prefix}[data-chat-flow-kind="assistant-step"] { ${paletteVariables(colors)} }`;
}

function semanticAccentCss(): string {
...
export function buildAccentCss(palette: AccentPalette | null): string {
